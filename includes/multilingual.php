 ...
 * Hooks into the existing `easyrankly_hreflang_alternates` filter so that
 ...
 */
define( 'EASYRANKLY_ML_DB_VERSION', '1.0.0' );

/**
 * Option name tracking the installed ML table version.
 */
define( 'EASYRANKLY_ML_DB_VERSION_OPTION', 'easyrankly_ml_db_version' );
 ...
 * @return bool
 */
function easyrankly_multilingual_enabled(): bool {
	return ! empty( easyrankly_get_setting( 'enable_multilingual' ) );
}

/**
 * Boots the multilingual module.
 *
 * Called from easyrankly_bootstrap() when is_multisite() and the feature is on.
 * Idempotent: repeated calls (e.g. via easyrankly_ml_admin()) are no-ops so
 * runtime hooks are never registered twice.
 *
 * @return void
 */
function easyrankly_ml_boot(): void {
	static $booted = false;
	...
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-activator.php';
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-sites.php';
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-repository.php';
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-resolver.php';
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-admin.php';
	require_once EASYRANKLY_PATH . 'includes/multilingual/class-easyrankly-ml-shortcodes.php';
	...
	wp_cache_add_global_groups( 'easyrankly_ml' );

	easyrankly_ml_maybe_upgrade_db();

	$repo     = new EasyRankly_ML_Repository();
	$resolver = new EasyRankly_ML_Resolver( $repo );
	...
	add_filter( 'easyrankly_hreflang_alternates', array( $resolver, 'resolve' ), 20 );

	// Visitor-facing consumers (easyrankly_get_navigable_hreflang_alternates(),
	// used by add-on language-redirect features) resolve through this instance so
	// they can include noindex translations.
	$GLOBALS['easyrankly_ml_resolver'] = $resolver;
	...
	$admin = new EasyRankly_ML_Admin( $repo );
	...
	$GLOBALS['easyrankly_ml_admin'] = $admin;

	( new EasyRankly_ML_Shortcodes( $repo ) )->register_hooks();
}
 ...
 *
 * @return EasyRankly_ML_Admin|null Instance, or null when the feature is off.
 */
function easyrankly_ml_admin(): ?EasyRankly_ML_Admin {
	if ( ! is_multisite() || ! easyrankly_multilingual_enabled() ) {
		return null;
	}
	...
	return $admin instanceof EasyRankly_ML_Admin ? $admin : null;
}
 ...
 * @return void
 */
function easyrankly_ml_maybe_upgrade_db(): void {
	$installed = (string) get_site_option( EASYRANKLY_ML_DB_VERSION_OPTION, '' );

	if ( EASYRANKLY_ML_DB_VERSION === $installed ) {
		return;
	}

	EasyRankly_ML_Activator::activate();
	update_site_option( EASYRANKLY_ML_DB_VERSION_OPTION, EASYRANKLY_ML_DB_VERSION );
}
 ...
 * @return void
 */
function easyrankly_ml_render_post_translations( WP_Post $post ): void {
	$admin = easyrankly_ml_admin();
	if ( $admin instanceof EasyRankly_ML_Admin ) {
 ...
 * @return void
 */
function easyrankly_ml_render_term_translations( int $term_id, string $taxonomy ): void {
	$admin = easyrankly_ml_admin();
	if ( $admin instanceof EasyRankly_ML_Admin ) {
 ...
 * @return void
 */
function easyrankly_ml_render_network_panel(): void {
	$admin = easyrankly_ml_admin();
	if ( $admin instanceof EasyRankly_ML_Admin ) {
